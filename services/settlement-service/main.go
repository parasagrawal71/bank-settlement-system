...
func main() {
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		fmt.Fprintln(w, "Welcome to Settlement HTTP Server!")
	})

	server := &http.Server{
		Addr:              fmt.Sprintf(":%s", env.GetEnvString("SETTLEMENT_GRPC_PORT", "")),
		ReadHeaderTimeout: 5 * time.Second,
	}

	log.Printf("✅ Server listening on http://localhost:%s", env.GetEnvString("SETTLEMENT_GRPC_PORT", ""))
